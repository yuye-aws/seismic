...
def convert_groundtruth_format(input_file, output_file):
    """Convert MS MARCO groundtruth format (comma-separated) to TSV format expected by Seismic"""
...
def convert_qrels_format(input_file, output_file):
    """Convert MS MARCO qrels format to add the 'useless' column with zero values"""
...
def main():
    parser = argparse.ArgumentParser(description="Convert MS MARCO CSR format to Seismic binary format")
    parser.add_argument("--input-dir", required=True, help="Input directory containing MS MARCO CSR files")
    ...
    # Convert documents
    docs_file = os.path.join(input_dir, "docs.csr")
    ...
    # Process other files with MS MARCO naming
    # Convert doc IDs
    doc_ids_file = os.path.join(input_dir, "doc_ids.txt")
    if os.path.exists(doc_ids_file):
        print(f"Converting {doc_ids_file} to doc_ids.npy")
        # For MS MARCO, doc IDs are sequential integers starting from 0
        # We can just create an array of integers
        with open(doc_ids_file, 'r') as f:
            doc_ids = [line.strip() for line in f]
        # Convert to numpy array of integers
        doc_ids_array = np.array([int(doc_id) for doc_id in doc_ids], dtype=np.int64)
        np.save(os.path.join(data_dir, "doc_ids.npy"), doc_ids_array)
        print(f"Saved {len(doc_ids_array)} document IDs")
    ...
    ground_truth_file = os.path.join(input_dir, "cocondense_ground_truth.txt")
    if os.path.exists(ground_truth_file):
        convert_groundtruth_format(ground_truth_file, os.path.join(data_dir, "groundtruth.tsv"))
    else:
        print(f"Warning: {ground_truth_file} not found")
    
    # Convert qrels file
    qrels_file = os.path.join(input_dir, "qrels.tsv")
    if os.path.exists(qrels_file):
        dst = os.path.join(output_dir, "qrels.msmarco.tsv")
        convert_qrels_format(qrels_file, dst)
        print(f"Converted {qrels_file} to {dst} (added useless column)")
    else:
